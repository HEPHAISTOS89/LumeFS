...
final class MigrationPlannerTests: XCTestCase {
    ...
    func testControllerPlansConfirmsCopiesAndJournalsEachStep() async throws {
        ...
        // ISO-8601 persistence drops fractional seconds, so compare the recorded facts, not Date equality.
        let persisted = persistence.load()
        XCTAssertEqual(persisted.entries.map(\.id), controller.journal.entries.map(\.id), "every step is written to disk")
        XCTAssertEqual(persisted.entries.map(\.kind), [.planned, .started, .completed])
        XCTAssertEqual(persisted.entries.map(\.planID), Array(repeating: plan.id, count: 3))
        XCTAssertEqual(persisted.entries.map(\.destinationPath), Array(repeating: plan.destinationURL.path, count: 3))
        XCTAssertEqual(persisted.entries.last?.fileCount, 3)
        XCTAssertEqual(persisted.entries.last?.byteCount, plan.inventory.totalBytes)
